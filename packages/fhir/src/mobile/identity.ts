 ...
 *
 * Only the system is barred from containing a vertical bar, so the name splits at the first one.
 * A value may contain them, because a composed identifier is built from them.
 */
export const entryIdentifierName = (
  ...
  if (input.system.includes('|')) {
    return err(
      'invalid-identifier',
      'Identifier.system must not contain a vertical bar, so the name splits at the first one.',
      ['system'],
    )
  }
  return ok(`${input.system}|${input.value}`)
}
