...
const missingAggregationMethod = (
  ...
  'methodChoice' in sharedMobileMeasurementCatalog[kind] &&
...
const makeObservation = (
  ...
  return {
    ...
    // A one-to-one conversion identifies its Observation by the source record, so the entry
    // identity is that same pair; listing it twice would repeat one identifier on the resource.
    identifier: [
      identifier(identities.sourceRecord),
      ...((
        observationIdentity.identifier.system ===
          identities.sourceRecord.system &&
        observationIdentity.identifier.value === identities.sourceRecord.value
      ) ?
        []
...
const resolveGraphIdentities = (
  ...
  // A one-to-one conversion mints no output identity, so its single Observation is identified by
  // the source record itself.
  const outputIdentities =
    connected.value.outputs.length > 0 ?
      connected.value.outputs
    : [connected.value.sourceRecord]
  const observations: IdentifiedEntryIdentityInput[] = []
  for (const [index, output] of outputIdentities.entries()) {
    ...
    observationEntries.push(
      identifiedEntry(
        ...
        makeObservation(
          ...
          identity,
          identities.value,
          aggregationMethodFor(validatedInput, measurement.kind),
        ),
      ),
    )
  }
