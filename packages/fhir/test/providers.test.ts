...
describe('Provider R4 graph builder', () => {
  ...
  it('derives exact source, conversion and exchange identifiers in their proper namespaces', () => {
    ...
    for (const identity of observation?.identifier ?? []) {
      expect(identity.value).toMatch(/^(v1:[^|]+(\|[^|]+)+|[^|]+)$/u)
    }
    ...
    // The provider's own record key travels in the clear: it is a vendor row key, not
    // participant data, and the same value is already public when the vendor's app writes it
    // into HealthKit or Health Connect.
    expect(JSON.stringify(result.value)).toContain(input.source.sourceNativeId)
    // The account component is a deployment-minted pseudonym, never a vendor-side account key.
    expect(input.source.providerAccountIdentifier.value).toMatch(/^pseudonym-/u)
  })
  ...
  it('matches the frozen Provider composed source and output vectors', () => {
    ...
    // Google Health is account-scoped, and a single-measurement record is a one-to-one
    // conversion, so the source record is the Observation's only identity.
    expect(observation?.identifier?.map((entry) => entry.value)).toEqual([
      'v1:google-health-api|account-001|steps|steps-2026-08-20T16:00:00Z',
    ])
    ...
    // Oura documents its document ids as unique across every account, so the key is carried
    // through exactly as Oura supplied it, non-ASCII included.
    expect(unicodeObservation?.identifier?.[0]?.value).toBe('résumé-\u0001')
  })
